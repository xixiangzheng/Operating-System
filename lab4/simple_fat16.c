...
cluster_t read_fat_entry(cluster_t clus)
{
    ...
    sector_t sec = meta.fat_sec + (clus * 2) / PHYSICAL_SECTOR_SIZE;
    off_t off = (clus * 2) % PHYSICAL_SECTOR_SIZE;
    int ret = sector_read(sec, sector_buffer);
    if(ret < 0) {
        return ret;
    }
    cluster_t clus_buff;
    memcpy(&clus_buff, sector_buffer + off, sizeof(cluster_t));

    // ===================================================
    return clus_buff; // TODO: 记得删除或者修改这一行
}
...
int find_entry_in_sectors(const char* name, size_t len, 
    ...
    for(size_t i=0; i < sectors_count; i++) {
        ...
        for(size_t off = 0; off < PHYSICAL_SECTOR_SIZE; off += DIR_ENTRY_SIZE ) {
...
int find_entry_internal(const char* path, DirEntrySlot* slot, const char** remains) {
    ...
    *remains = next_level;
    while (true) {
        ...
        // ================== Your code here =================
        while(true){
            first_sec = cluster_first_sector(clus); // 获取当前簇的第一个扇区
            nsec = meta.sec_per_clus;   // 当前簇有多少个扇区
            state = find_entry_in_sectors(*remains, len, first_sec, nsec, slot);    // 在当前簇的所有扇区中查找
            if (state == FIND_FULL){    // 如果当前簇已满且未找到
                clus = read_fat_entry(clus);    // 获取目录的下一个簇
            }
            else {
                break;
            }
...
int fill_entries_in_sectors(sector_t first_sec, size_t nsec, fuse_fill_dir_t filler, void* buf) {
    ...
    for(size_t i=0; i < nsec; i++) {
        ...
        for(size_t off = 0; off < PHYSICAL_SECTOR_SIZE; off += DIR_ENTRY_SIZE ) { // TODO: 请补全循环条件（每个扇区多大？目录项多大？）
...
int read_from_cluster_at_offset(cluster_t clus, off_t offset, char* data, size_t size) {
     ...
     */
    uint32_t sec = cluster_first_sector(clus) + offset / PHYSICAL_SECTOR_SIZE; // TODO: 请填写正确的扇区号。
    size_t sec_off = offset % PHYSICAL_SECTOR_SIZE; // TODO: 请填写正确的扇区内偏移量。
    ...
    while(pos < size) { // 还没有读取完毕
        ...
        // ================== Your code here =================
        size_t len;
        if(size - pos > PHYSICAL_SECTOR_SIZE- sec_off){
            len = PHYSICAL_SECTOR_SIZE - sec_off;
        }else{
            len = size - pos;
        }
...
int fat16_read(const char *path, char *buffer, size_t size, off_t offset,
    ...
    while(is_cluster_inuse(clus) & (p < size)){
        size_t len;
        if(offset + size - p > meta.cluster_size){
            len = meta.cluster_size - offset;
        }else{
            len = size - p;
        }
        int ret = read_from_cluster_at_offset(clus, offset, buffer + p, len);
        if(ret < 0){
            return ret;
        }
        p += len;
...
int dir_entry_create(DirEntrySlot slot, const char *shortname, 
    ...
    // ================== Your code here =================
    memcpy(dir->DIR_Name, shortname, FAT_NAME_LEN);
    dir->DIR_Attr = attr;
    dir->DIR_FstClusHI = 0;
...
int write_fat_entry(cluster_t clus, cluster_t data) {
    ...
    for(size_t i = 0; i < meta.fats; i++) {
         ...
         */
        // ================== Your code here =================
        sector_t fat_sec = i * meta.sec_per_fat + meta.fat_sec + clus_sec;
        int ret = sector_read(fat_sec, sector_buffer);
        if(ret < 0){
            return ret;
        }
        memcpy(sector_buffer + sec_off, &data, sizeof(cluster_t));
        ret = sector_write(fat_sec, sector_buffer);
        if(ret < 0){
            return ret;
        }
        // ===================================================
...
int alloc_one_cluster(cluster_t* clus) {
    ...
    for(cluster_t i = 2; i < meta.clusters; i++){
        cluster_t clus_fat = read_fat_entry(i);
        if (clus_fat == CLUSTER_FREE){
...
int alloc_clusters(size_t n, cluster_t* first_clus) {
    ...
    // ================== Your code here =================
    size_t free_clusters_num = 0;
    for(cluster_t i = 2 ; i < meta.clusters; i++){
        if(read_fat_entry(i) == CLUSTER_FREE){
            clusters[free_clusters_num] = i;
            free_clusters_num ++;
        }
        if(free_clusters_num == n){
            break;
        }
    }
    ...
    clusters[free_clusters_num] = CLUSTER_END;
...
int fat16_mkdir(const char *path, mode_t mode) {
    ...
    // ================== Your code here =================
    ret = alloc_one_cluster(&dir_clus);  // 分配一个空闲簇给新目录
    if(ret < 0){
        return ret;
    }
    ret = find_empty_slot(path, &slot, &filename);  // 在父目录中找到一个空槽来创建新目录的条目
    if(ret < 0){
        return ret;
    }
    char shortname[11];
    ret = to_shortname(filename, MAX_NAME_LEN, shortname);  // 将新目录名转换为短文件名
    if(ret < 0){
        return ret;
    }
    ret = dir_entry_create(slot, shortname, ATTR_DIRECTORY, dir_clus, 0);   // 在父目录的空槽 slot 处，创建一个目录项，指向新分配的簇 dir_clus
    if(ret < 0){
        return ret;
    }
    // ===================================================
...
int fat16_rmdir(const char *path) {
    ...
    DirEntrySlot slot = {{}, 0, 0};
    DIR_ENTRY* dir = &(slot.dir);
    int ret = find_entry(path, &slot); // 1. 找到要删除目录的目录项
    if(ret < 0){
        return ret;
    }
    if(!attr_is_directory(dir->DIR_Attr)){
    ...
    const char DOT_NAME[] =    ".          ";
    const char DOTDOT_NAME[] = "..         ";
    cluster_t clus = dir->DIR_FstClusLO;    // 目录的第一个数据簇
    while (is_cluster_inuse(clus)) {
        sector_t first_sec = cluster_first_sector(clus);
        size_t nsec = meta.sec_per_clus;
        for(size_t i = 0; i < nsec; i++){   // 遍历簇中的每个扇区
            sector_t sec = first_sec + i;   
            ...
            for(size_t off = 0; off < PHYSICAL_SECTOR_SIZE; off += DIR_ENTRY_SIZE) { 
                DIR_ENTRY* entry = (DIR_ENTRY*)(sector_buffer + off);
                if(de_is_valid(entry)) {    // 如果是有效目录项
                    //if((check_name(DOT_NAME, FAT_NAME_LEN, entry) != 1) && (check_name(DOTDOT_NAME, FAT_NAME_LEN, entry) != 1)){
                    if((strncmp(DOT_NAME, (const char *)entry->DIR_Name, FAT_NAME_LEN) != 0) && (strncmp(DOTDOT_NAME, (const char *)entry->DIR_Name, FAT_NAME_LEN) != 0)){
                        return -ENOTEMPTY;
                    }
                }
            }
...
ssize_t write_to_cluster_at_offset(cluster_t clus, off_t offset, const char* data, size_t size) {
    ...
    size_t pos = 0;
    while(pos < size) {
        int ret = sector_read(sec, sector_buffer);  // 读取当前扇区
        if(ret < 0) {
            return ret;
        }
        size_t len;
        if(size - pos > PHYSICAL_SECTOR_SIZE - sec_off){    // 如果剩余要写的比当前扇区剩余空间大, 则本次写满当前扇区剩余空间
            len = PHYSICAL_SECTOR_SIZE - sec_off;
        }else{
            len = size - pos;
        }
        memcpy(sector_buffer+sec_off, data+pos, len);   // 将数据从 data 复制到扇区缓冲区的正确位置
        ret = sector_write(sec, sector_buffer);     // 将修改后的整个扇区写回磁盘
        if(ret < 0){
            return ret;
        }
        pos += len;
...
int fat16_write(const char *path, const char *data, size_t size, off_t offset,
    ...
    cluster_t clus = dir->DIR_FstClusLO;
    size_t clus_num = 0;
    while(is_cluster_inuse(clus)){  // 只有当文件已有簇时才计算
        clus_num ++;
        clus = read_fat_entry(clus);
    }
    ...
    // 2. 判断是否需要分配新的簇
    if(offset + size > clus_num * meta.cluster_size){
        size_t add_clus_num = (offset + size - clus_num * meta.cluster_size) / meta.cluster_size + 1;
        cluster_t new_first_clus;
        ret = alloc_clusters(add_clus_num, &new_first_clus);    // 分配所需的新簇
        if(ret < 0){
            return ret;
        }

        // 3. 将新分配的簇链接到文件现有簇链的末尾
        while(true){
            if(!clus){
                // 找到文件当前的最后一个簇
            ...
            // 将最后一个旧簇的FAT表项指向新分配簇链的第一个簇
            if(read_fat_entry(clus) == CLUSTER_END){
                ret = write_fat_entry(clus, new_first_clus);
                if(ret < 0){
                    return ret;
                }
                break;
    ...
    while(p < size){
        size_t len;
        if(size - p > meta.cluster_size - offset){  // 如果要写的超过当前簇剩余空间
            len = meta.cluster_size - offset;
        }else{
            len = size - p;
        }
        // 调用 write_to_cluster_at_offset
        ret = write_to_cluster_at_offset(clus, offset, data + p, len);
        if(ret < 0){
            return ret;
        }
        p += len;
